...
@section('content')
    <div x-data="{ open: false, deleteUrl: '' }" class="bg-white p-4 md:p-6 rounded shadow-md">
            ...
            <div class="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-red-600" fill="none" viewBox="0 0 24 24"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M3 21V10l6 4V10l6 4V6h6v15H3z" />
                </svg>
                <h3 class="text-xl md:text-2xl font-bold text-gray-700">Plant</h3>
            </div>
            <a href="{{ route('master-plant.create') }}"
        ...
        {{-- Filter --}}
        <form method="GET" action="{{ route('master-plant.index') }}" class="mb-6">
            ...
            <table class="min-w-max w-full bg-white border border-gray-200 text-sm text-gray-700">
                <thead class="bg-gray-100 text-center uppercase tracking-wide">
                    <tr>
                        ...
                        <th class="px-4 py-2 border text-left">Plant Name</th>
                        <th class="px-4 py-2 border text-left">BU Code</th>
                ...
                <tbody>
                    @forelse ($plants as $index => $plant)
                        <tr class="hover:bg-gray-50 text-center">
                            <td class="px-4 py-2 border">{{ $index + $plants->firstItem() }}</td>
                            <td class="px-4 py-2 border text-left">{{ $plant->plant_code }}</td>
                            <td class="px-4 py-2 border text-left">{{ $plant->plant_name }}</td>
                            <td class="px-4 py-2 border text-left">{{ $plant->bu_code }}</td>
                            <td class="px-4 py-2 border">
                                <span
                                    class="inline-block px-2 py-1 text-xs font-semibold rounded
                                {{ $plant->isactive === 'T' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            ...
                            <td class="px-4 py-2 border text-center">
                                <div class="flex items-center justify-center gap-x-3">
                                    <!-- Tombol Edit -->
                                    <a href="{{ route('master-plant.edit', $plant->plant_code) }}"
                                    ...
                                    <!-- Tombol Delete -->
                                    <button type="button"
                                        @click="open = true; deleteUrl = '{{ route('master-plant.destroy', $plant->plant_code) }}'"
        ...
        @if ($plants->count())
            <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-gray-600">
                <div class="mb-2 sm:mb-0">
                    Showing {{ $plants->firstItem() }} to {{ $plants->lastItem() }} of {{ $plants->total() }} entries
                </div>

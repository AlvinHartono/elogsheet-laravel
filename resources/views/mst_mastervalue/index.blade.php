...
@section('page_title', 'Master Value')
...
@section('content')
    <div x-data="{ open: false, deleteUrl: '' }" class="bg-white p-4 md:p-6 rounded shadow-md">
            ...
            <div class="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-red-600" fill="none" viewBox="0 0 24 24"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M3 21h18M5 21V9a1 1 0 011-1h4v13M14 21V5a1 1 0 011-1h4v17" />
                </svg>
                <h3 class="text-xl md:text-2xl font-bold text-gray-700">Value</h3>
            </div>
            <a href="{{ route('master-value.create') }}"
        ...
        {{-- Filter --}}
        <form method="GET" action="{{ route('master-value.index') }}" class="mb-6">
        ...
        <div class="overflow-x-auto rounded shadow">
            <table class="min-w-max w-full bg-white border border-gray-200 text-sm text-gray-700">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($mastervalues as $index => $value)
                        <tr class="hover:bg-gray-50 text-center">
                            <td class="px-4 py-2 border">{{ $index + $mastervalues->firstItem() }}</td>
                            <td class="px-4 py-2 border text-left">{{ $value->code }}</td>
                            <td class="px-4 py-2 border text-left">{{ $value->name }}</td>
                            <td class="px-4 py-2 border text-left">{{ $value->group }}</td>
                            <td class="px-4 py-2 border">
                                <span
                                    class="inline-block px-2 py-1 text-xs font-semibold rounded
                                {{ $value->isactive === 'T' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                    {{ $value->isactive === 'T' ? 'Aktif' : 'Tidak Aktif' }}
                                </span>
                            </td>
                            <td class="px-4 py-2 border text-center">
                                <div class="flex items-center justify-center gap-x-3">
                                    <!-- Tombol Edit -->
                                    <a href="{{ route('master-value.edit', $value->id) }}"
                                    ...
                                    <!-- Tombol Delete -->
                                    <button type="button"
                                        @click="open = true; deleteUrl = '{{ route('master-value.destroy', $value->id) }}'"
        ...
        {{-- Custom Pagination --}}
        @if ($mastervalues->count())
            <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-gray-600">
                <div class="mb-2 sm:mb-0">
                    Showing {{ $mastervalues->firstItem() }} to {{ $mastervalues->lastItem() }} of
                    {{ $mastervalues->total() }} entries
                </div>

                <div class="flex items-center space-x-2">
                    {{-- Previous --}}
                    @if ($mastervalues->onFirstPage())
                        <span class="px-3 py-1 bg-gray-200 text-gray-500 rounded cursor-not-allowed">Previous</span>
                    @else
                        <a href="{{ $mastervalues->previousPageUrl() }}"
                            class="px-3 py-1 bg-white border border-gray-300 rounded hover:bg-gray-100">Previous</a>
                    @endif

                    {{-- Page Info --}}
                    <span class="px-3 py-1">
                        Page {{ $mastervalues->currentPage() }} of {{ $mastervalues->lastPage() }}
                    </span>

                    {{-- Next --}}
                    @if ($mastervalues->hasMorePages())
                        <a href="{{ $mastervalues->nextPageUrl() }}"

...
@section('page_title', 'Master Business Unit')
    ...
    <div x-data="{ open: false, deleteUrl: '' }" class="bg-white p-4 md:p-6 rounded shadow-md">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div class="flex items-center space-x-2">
                ...
                <h3 class="text-xl md:text-2xl font-bold text-gray-700">Business Unit</h3>
            </div>
        ...
        {{-- Filter --}}
        <form method="GET" action="{{ route('business-unit.index') }}" class="mb-6">
        ...
        <div class="overflow-x-auto rounded shadow">
            <table class="min-w-max w-full bg-white border border-gray-200 text-sm text-gray-700">
                ...
                <tbody>
                    @forelse ($businessUnits as $index => $unit)
                        <tr class="hover:bg-gray-50 text-center">
                            <td class="px-4 py-2 border">{{ $index + $businessUnits->firstItem() }}</td>
                            <td class="px-4 py-2 border text-left">{{ $unit->bu_code }}</td>
                            <td class="px-4 py-2 border text-left">{{ $unit->bu_name }}</td>
                            <td class="px-4 py-2 border">
                                <span
                                    class="inline-block px-2 py-1 text-xs font-semibold rounded
                                {{ $unit->isactive === 'T' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                    {{ $unit->isactive === 'T' ? 'Aktif' : 'Tidak Aktif' }}
                                </span>
                            </td>
                            <td class="px-4 py-2 border text-center">
                                <div class="flex items-center justify-center gap-x-3">
                                    <!-- Tombol Edit -->
                                    <a href="{{ route('business-unit.edit', $unit->bu_code) }}"
                                    ...
                                    <!-- Tombol Delete -->
                                    <button type="button"
                                        @click="open = true; deleteUrl = '{{ route('business-unit.destroy', $unit->bu_code) }}'"
        ...
        @if ($businessUnits->count())
            <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-gray-600">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @else
                        <a href="{{ $businessUnits->previousPageUrl() }}"
                            class="px-3 py-1 bg-white border border-gray-300 rounded hover:bg-gray-100">Previous</a>
                    @endif

                    {{-- Page Info --}}
                    <span class="px-3 py-1">
                        Page {{ $businessUnits->currentPage() }} of {{ $businessUnits->lastPage() }}
                    </span>

                    {{-- Next --}}
                    @if ($businessUnits->hasMorePages())
                        <a href="{{ $businessUnits->nextPageUrl() }}"

...
@section('page_title', 'Report Pretreatment Bleaching')
...
@section('content')
    ...
    <div class="bg-white p-6 rounded shadow-md">
        <div class="flex items-center justify-between mb-6">
            <div>
                <div class="flex items-center space-x-3 mb-1">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-800">Pretreatment Bleaching & Filtration</h2>
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('report-pretreatment.export.excel', ['filter_tanggal' => $tanggal]) }}" target="_blank"
                ...
                <a href="{{ route('report-pretreatment.export.view', ['filter_tanggal' => $tanggal, 'filter_refinery_machine' => request('filter_refinery_machine')]) }}"
                ...
                <a href="{{ route('report-pretreatment.export.pdf', ['filter_tanggal' => $tanggal, 'filter_refinery_machine' => request('filter_refinery_machine')]) }}"
        ...
        <div class="bg-gray-50 p-4 rounded-md shadow-sm mb-6">
            <form method="GET" action="{{ route('report-pretreatment.index') }}" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if (request()->has('filter_tanggal') || request()->has('filter_jam') || request()->has('filter_refinery_machine'))
        ...
        <div x-data="{ openRejectModal: false }">
            <div class="flex gap-2 mb-4">
                <form action="{{ route('report-pretreatment.approve-date') }}" method="POST">
                    @csrf
            ...
            <div x-show="openRejectModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
                ...
                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
                    ...
                    <form action="{{ route('report-pretreatment.reject-date') }}" method="POST">
                        @csrf
                        <input type="hidden" name="posting_date" value="{{ $tanggal }}">
                        <div class="mb-4">
                ...
                <tbody class="text-sm text-gray-700">
                    @forelse ($reports as $index => $report)
                        <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100">
                            ...
                            <td class="px-4 py-2 border-b">{{ \Carbon\Carbon::parse($report->time)->format('H.i') }}</td>
                            ...
                            <td class="px-4 py-2 border-b text-center">
                                <div class="flex justify-center gap-2" x-data="{ showApprove: false, showReject: false }">
                                    {{-- Approve/Reject for Leader --}}
                                    @if (auth()->user()->roles === 'LEAD_PROD' || auth()->user()->roles === 'LEAD')
                                        <button @click="showApprove = true"
                                            class="px-3 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 shadow {{ !is_null($report->prepared_status) ? 'opacity-50 cursor-not-allowed' : '' }}"
                                            {{ !is_null($report->prepared_status) ? 'disabled' : '' }}>Approve</button>
                                        <button @click="showReject = true"
                                            class="px-3 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 shadow {{ !is_null($report->prepared_status) ? 'opacity-50 cursor-not-allowed' : '' }}"
                                            {{ !is_null($report->prepared_status) ? 'disabled' : '' }}>Reject</button>
                                    @endif
                                    {{-- Approve/Reject for Manager --}}
                                    @if (auth()->user()->roles === 'MGR_PROD' || auth()->user()->roles === 'MGR')
                                        <button @click="showApprove = true"
                                            class="px-3 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 shadow {{ !is_null($report->checked_status) ? 'opacity-50 cursor-not-allowed' : '' }}"
                                            {{ !is_null($report->checked_status) ? 'disabled' : '' }}>Approve</button>
                                        <button @click="showReject = true"
                                            class="px-3 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 shadow {{ !is_null($report->checked_status) ? 'opacity-50 cursor-not-allowed' : '' }}"
                                            {{ !is_null($report->checked_status) ? 'disabled' : '' }}>Reject</button>
                                    @endif
                                    ...
                                    <div x-show="showApprove"
                                        ...
                                        <div class="bg-white p-6 rounded-lg shadow-xl">
                                            ...
                                            <p>Are you sure you want to approve ticket #{{ $report->id }}?</p>
                                            <div class="mt-6 flex justify-end gap-2">
                                                <button @click="showApprove = false"
                                                    class="px-4 py-2 bg-gray-300 rounded">Cancel</button>
                                                <form method="POST"
                                                    action="{{ route('report-pretreatment.approve', $report->id) }}">
                                                    @csrf
                                                    <button type="submit"
                                    ...
                                    <div x-show="showReject"
                                        ...
                                        <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                                            ...
                                            <form method="POST"
                                                action="{{ route('report-pretreatment.reject', $report->id) }}">
                                                @csrf
                            ...
                            <td class="px-4 py-2 border-b text-center">
                                <a href="{{ route('report-pretreatment.show', $report->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="12" class="px-4 py-4 text-center text-gray-500 text-sm">Data tidak tersedia.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
